...
const QuickActionsDropdown: React.FC<
  Pick<HeaderProps, 'onOpenAddCameraModal' | 'onStartAllRecording' | 'onStopAllRecording'>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-black/90 border-2 border-[var(--color-neon-purple)] rounded-md shadow-lg z-20">
          <ul className="py-1">
            ...
            <li>
              <button
                onClick={() => handleAction(onOpenAddCameraModal)}
                className="w-full text-left flex items-center space-x-3 px-3 py-2 hover:bg-[var(--color-neon-cyan)]/20"
              >
            ...
            <li>
              <button className="w-full text-left flex items-center space-x-3 px-3 py-2 hover:bg-[var(--color-neon-cyan)]/20">
            ...
            <li>
              <button
                onClick={() => handleAction(onStartAllRecording)}
                className="w-full text-left flex items-center space-x-3 px-3 py-2 hover:bg-[var(--color-neon-cyan)]/20 text-[var(--color-neon-pink)]"
              >
                <RecordIcon className="w-5 h-5" />
                <span>Start All Recording</span>
              </button>
            </li>
            <li>
              <button
                onClick={() => handleAction(onStopAllRecording)}
                className="w-full text-left flex items-center space-x-3 px-3 py-2 hover:bg-[var(--color-neon-cyan)]/20"
              >
                <RecordIcon className="w-5 h-5" />
                <span>Stop All Recording</span>
              </button>
            </li>
          </ul>
        </div>
...
const UserMenu: React.FC<Pick<HeaderProps, 'userName' | 'onLogout'>> = ({ userName = 'Operator', onLogout }) => {
  const [isOpen, setIsOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="flex items-center space-x-2 p-2 rounded-md hover:bg-[var(--color-neon-cyan)]/20"
        title="User Menu"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-black/90 border-2 border-[var(--color-neon-purple)] rounded-md shadow-lg z-30">
          <div className="px-3 py-2 border-b border-[var(--color-neon-purple)] text-xs text-gray-400 uppercase">
            Session
          </div>
          <button
            className="w-full text-left px-3 py-2 hover:bg-[var(--color-neon-cyan)]/20 text-sm"
            onClick={onLogout}
          >
            Sign out
          </button>
...
export const Header: React.FC<HeaderProps> = (props) => {
  ...
  return (
    <header className="flex-shrink-0 flex items-center justify-between p-3 bg-black/70 border-b-2 border-[var(--color-neon-purple)]">
      <h1 className="font-orbitron font-bold text-2xl animated-title-glow">CYBERSEC COMMAND</h1>
      <div className="flex items-center space-x-2">
        ...
        <div className="w-px h-8 bg-[var(--color-neon-purple)] mx-2" />

        <button
          onClick={() => setIsRemoteMode((prev) => !prev)}
          title={isRemoteMode ? 'Switch to LAN Mode' : 'Switch to Remote Mode'}
          className="p-2 rounded-md hover:bg-[var(--color-neon-cyan)]/20"
        >
        ...
        <button className="p-2 rounded-md hover:bg-[var(--color-neon-cyan)]/20 relative" title="Notifications">
          ...
          <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-[var(--color-neon-pink)]" />
        </button>
